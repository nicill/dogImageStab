...
void Parameters::process() {
    ...
    while ((opt = getopt(argc, argv, "i:o:s:m:b:N:f:x:y:d:zl")) != EOF) {
        switch (opt) {
            ...
            case '?': throw string("Illegal option: ") + string(argv[optind-1]);
            default:  throw string("No handler for option: ") + string(argv[optind-1]);
        }
    }
